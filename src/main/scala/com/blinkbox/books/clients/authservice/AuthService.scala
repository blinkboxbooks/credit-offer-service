...
import spray.http.{FormData, HttpResponse, HttpEntity}
...
class AuthServiceClient(cfg: AuthServiceClientConfig) extends AuthService
  ...
  private val serviceUrl = s"${cfg.url}/oauth2/token"
  ...
  override def authenticate(): Future[AuthTokens] = {
    val reqData = Map("grant_type" -> "password", "username" -> cfg.username, "password" -> cfg.password)
    ...
    call(Post(serviceUrl, FormData(reqData))(FormDataMarshaller), responseHandler)
  }
  ...
  private val unmarshalResponse: HttpEntity => AuthTokens = unmarshal(json4sUnmarshaller[AuthTokens])

  private def responseHandler: PartialFunction[HttpResponse, AuthTokens] = {
